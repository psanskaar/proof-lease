...
import { Server, CheckCircle, ExternalLink, ArrowLeft, Loader2, Plus, AlertCircle, Clock, Wallet } from 'lucide-react'
...
function WithdrawPanel({ address }: { address: `0x${string}` }) {
  ...
  const handleWithdraw = () => {
    writeContract({ address: ESCROW, abi: ESCROW_ABI, functionName: 'withdraw' })
  }
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {isSuccess && (
          <span className="text-xs text-green-400 flex items-center gap-1">
            <CheckCircle size={12}/>Withdrawn!
          </span>
        )}
      </div>

      <div className="bg-gray-950 rounded-lg p-4 flex items-center justify-between mb-4">
        <div>
          <div className="text-xs text-gray-500 mb-1">Available to withdraw</div>
          <div className={`text-2xl font-bold font-mono ${hasBalance ? 'text-green-400' : 'text-gray-600'}`}>
            {formatEther(amount)} BOT
          </div>
          <div className="text-xs text-gray-600 mt-1">
            Provider earnings + buyer refunds from LeaseEscrow
          </div>
        </div>
        <button
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        {!isConnected ? (
          ...
          <>
            {/* Withdraw panel — always visible when connected */}
        ...
        <div className="mt-6 bg-gray-900 rounded-xl p-4 border border-gray-800 text-xs text-gray-500 leading-relaxed">
          ...
          Withdraw anytime using the panel above.
        </div>
      </div>
    </div>
  )
